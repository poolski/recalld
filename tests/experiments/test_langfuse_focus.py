...
from recalld.experiments.langfuse_focus import (
    load_focus_experiment_context,
    run_focus_prompt_experiment,
)
...
def test_load_focus_experiment_context_reads_artifacts(tmp_path):
    job = _write_job_artifacts(tmp_path)

    context = load_focus_experiment_context(job.id, scratch_root=tmp_path)

    assert context.job_id == job.id
    assert context.dataset_name == f"recalld/jobs/{job.id}/focus"
    assert context.dataset_item_id == f"job-{job.id}-focus"
    assert context.input_data["speaker_b_name"] == "Lizzie"
    assert context.expected_output["focus_points"] == ["Keep working"]


def test_run_focus_prompt_experiment_registers_dataset_filter(tmp_path, monkeypatch):
    job = _write_job_artifacts(tmp_path)
    context = load_focus_experiment_context(job.id, scratch_root=tmp_path)
    dataset = _FakeDataset(context.input_data, context.expected_output, dataset_id="dataset-focus")
    ...
    monkeypatch.setattr("recalld.experiments.langfuse_focus.get_langfuse_client", fake_get_client)
    monkeypatch.setattr("recalld.experiments.langfuse_focus.shutdown_tracing", lambda: None)
    monkeypatch.setattr(
        "recalld.experiments.langfuse_focus.load_config",
    ...
    monkeypatch.setattr(
        "recalld.experiments.langfuse_focus.ensure_loaded_context_length",
        fake_context_length,
    )
    ...
    monkeypatch.setattr(
        "recalld.experiments.langfuse_focus.ensure_evaluator_rules_include_dataset",
        fake_register,
    )

    async def fake_generate_focus_points(*args, **kwargs):
        return {
            "focus_points": ["Keep working"],
            "strategy": "single",
            "topic_count": 1,
    ...
    monkeypatch.setattr("recalld.experiments.langfuse_focus._generate_focus_points", fake_generate_focus_points)
    ...
    assert captured == [("dataset-focus", client)]
    assert captured_envs == ["experiments"]
    assert "LANGFUSE_TRACING_ENVIRONMENT" not in os.environ


def test_run_focus_prompt_experiment_wraps_session_context(tmp_path, monkeypatch):
    job = _write_job_artifacts(tmp_path)
    context = load_focus_experiment_context(job.id, scratch_root=tmp_path)
    dataset = _FakeDataset(context.input_data, context.expected_output, dataset_id="dataset-focus")
    client = _FakeClient(dataset)
    ...
    monkeypatch.setattr("recalld.experiments.langfuse_focus.get_langfuse_client", lambda: client)
    monkeypatch.setattr("recalld.experiments.langfuse_focus.shutdown_tracing", lambda: None)
    monkeypatch.setattr(
        "recalld.experiments.langfuse_focus.session_context",
        lambda session_id: _FakeSessionContext(calls, session_id),
    )
    monkeypatch.setattr(
        "recalld.experiments.langfuse_focus.make_session_id",
    ...
    assert dataset.calls[0]["run_name"].startswith("focus · session · production · fallback-model · ctx1000")
    expected_session = f"run-session-{job.id[-6:]}"
    assert len(client.created_scores) == 2
    assert [score["name"] for score in client.created_scores] == ["reference_alignment", "focus-evaluator"]
